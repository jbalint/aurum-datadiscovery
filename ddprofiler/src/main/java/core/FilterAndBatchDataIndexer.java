...
public class FilterAndBatchDataIndexer implements DataIndexer {
	...
	/**
	 * TODO: For now simply make sure upserts work well. In the (near) future, prepare a filtering mechanism here, fixed to the context of a source that removes data per column
	 * that has already been indexed. Chances are we can reduce the amount of data to index a lot!
	 *
	 * Final version will do, per field: - filter out already seen data (data that has been seen with high probability) - batch data locally until there's an important amount
	 * (configurable) - then call the store (which may or may not batch the request)
	 *
	 * For now: - create update request on the same document to include more terms (all of them) - each call to indexData involves deleting, creating, reindexing doc, so we want to
